...
struct huffman {
    ...
    std::unordered_map<T, std::pair<uint64_t, uint64_t>> code_map;
    std::vector<std::tuple<uint64_t, T, uint64_t>> canonical;
...
void decompress(const std::string& input_filename,
    ...
    std::vector<std::tuple<uint64_t, uint8_t, uint64_t>> tbl;
